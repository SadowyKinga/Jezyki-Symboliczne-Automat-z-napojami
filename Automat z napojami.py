...
class ObslugaAutomatu(Produkty, Monety):
    ...
    *reszta - wydaje reszte za zakupiony towar, wyciąga monety ze skarbca na pieniądze lub informuje klienta by wrzucił odliczoną kwote, gdy nie jest w stanie wydać reszty. """
    ...
    def sprawdz(self):
        if  not 30 <= self.numer <= 50:  #range - to nie generator
            daj = 0
        else:
...
#--------------------- OBSLUGA PRZYCISKOW FUNKCYJNYCH ------------------------------------------
    ...
    def przycisk_do_czyszczenia(self):
        ...
        if self.wrzucone is not "0":
            ...
            string.set('Podaj numer z przedziału 30-50, a następnie co chcesz zrobić: \nZielony przycisk - rozpoczecie zakupu; \nCzerwony - anulowanie zakupu; \nNiebieski - sprawdzenie dostepnosci towaru. \n\nBy sprawdzić wartość energetyczną wybranego napoju \nnajedź na niego myszką i kliknij na wybrany produkt')
...
ramka_nr_1.place(relx = 0.05, rely = 0.01, relwidth = 0.5, relheight = 0.32) #relx - polożenie poziome, rely - połozenie pionowe, relwidth - rozmiar szerokosć, relheight - rozmiar wysokosc
